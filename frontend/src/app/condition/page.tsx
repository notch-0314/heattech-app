...
export default function Recommended() {
  ...
  const getConditionText = (score) => {
    if (score >= 85) return '最適';
    if (score >= 70) return '良好';
    if (score >= 60) return '適度';
    return '要注意';
  };

  const getColorForCondition = (conditionValue) => {
    if (conditionValue >= 85) return 'bg-blue-100';
    if (conditionValue >= 70) return 'bg-yellow-100';
    if (conditionValue >= 60) return 'bg-pink-500';
    return 'bg-red-500';
  };
    ...
    <div className="flex flex-col items-center p-4 h-screen font-sans">
      <div className="w-full flex-grow overflow-auto pb-24">
        ...
        <div className="w-full flex justify-between my-4">
          <div className={`w-1/2 p-6 text-center mx-2 ${getColorForCondition(previousScore)}`}>
            <p>Previous Score</p>
            <p className="my-2">
          ...
          <div className={`w-1/2 p-6 text-center mx-2 ${getColorForCondition(todayScore)}`}>
            <p>Today Score</p>
            <p className="my-2">
        ...
        <h2 className="text-lg font-semibold mb-2">コンディションコントリビューター</h2>
